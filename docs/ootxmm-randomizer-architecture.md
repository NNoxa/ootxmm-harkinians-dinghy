...
The staging manifest lives at `shared_assets/ootxmm/manifest.ootxmm-assets.json` and is copied into both extracted ports under `mods/OoTxMM/`. Renderer hooks should read `SharedPlacement.model` through each port's crossover model lookup and then resolve that id to either a native draw id or a packed shared display list.

## Mod Compatibility

The ports can load asset archives from their `mods` folders, but a generic `.otr` or `.o2r` file does not reliably tell OoTxMM that it added a randomizer item, how to grant that item, or which model should represent it in the other game. OoTxMM therefore uses an opt-in sidecar manifest for randomizer-aware mods.
